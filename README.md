...
Momento 模式比较适用于功能比较复杂的，但需要维护或记录属性历史的类，或者需要保存的属性只是众多属性中的一部分时，

Originator 可以根据保存的 Momento 信息还原到前一状态。
...
组合模式定义了包含基本对象和组合对象的类的层次关系。基本对象可以被组合成更复杂的组合对象，而这个组合对象又可以被组合，

这样不断地**递归**下去，客户代码中，任何用到基本对象的地方都可以使用组合对象了。
...
具体迭代器类（ConcreteIterator），继承 Iterator ，实现开始，下一个，是否结尾，当前对象等方法。

聚集抽象类（Aggregate）

具体聚集类（ConcreteAggregate），继承聚集抽象类
